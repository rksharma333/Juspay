...
void printTree(Node *r)
{
      ...
      for (auto k : r->links)
      {
            cout << k->v << " anc_locked: " << k->anc_locked << " dec_locked:" << k->dec_locked << " isLocked:" << k->isLocked << " uid:" << k->uid << "\n";
      }
      cout << "\n";
      for (auto k : r->links)
      {
...
public:
      ...
      void infromDecendants(Node *r, int val)
      {
            for (auto k : r->links)
            {
                  k->anc_locked += val;
                  infromDecendants(k, val);
            }
      }
